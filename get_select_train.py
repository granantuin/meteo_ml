...
#defining independent variable and label y
y_var = ["spd_o"] #@param ["[\"mod_o\"]", "[\"temp_o\"]", "[\"spd_o\"]", "[\"visibility_o\"]", "[\"dir_o\"]", "[\"dir_o\",\"spd_o\"]", "[\"wind_gust_o\"]", "[\"wxcodes_o\"]", "[\"skyc1_o\"]", "[\"skyl1_o\"]", "[\"temp_o\"]", "[\"tempd_o\"]", "[\"gust_spd_o\"]", "[\"prec_accumulated_1_hour_before_o\"]"] {type:"raw", allow-input: true}
...
#model=MLPClassifier(hidden_layer_sizes=(100,100 ),learning_rate="adaptive",verbose=False, 
                    #early_stopping=False,max_iter=3000,activation="relu",alpha=0.0001, solver="adam")
...
if x_and_y_same:
  met_ml={"scaler":scaler,"pca":pca,"model":model,"global":global_ml,"column":column_ml,
        "index":index_ml,"class":clas_ml,"met_var_sc":met_var_sc,"x_and_y_same":True,
        "abstract":abstract,"D":int(Database[-5:-4]),"interval":interval,"x_var":X.columns.to_list(),
        "y_var":y_var,"labels":labels,"cros_val":cros_val,"lat":df_r.lat.to_list(),
        "lon":df_r.lon.to_list(),"mod_res":"d03"}
else:
   met_ml={"scaler":scaler,"pca":pca,"model":model,"global":global_ml,"column":column_ml,
        "index":index_ml,"class":clas_ml,"x_and_y_same":False,"abstract":abstract,
        "D":int(Database[-5:-4]),"y_var":y_var,"x_var":X.columns.to_list(),
        "labels":labels,"cros_val":cros_val,"lat":df_r.lat.to_list(),
        "lon":df_r.lon.to_list(),"mod_res":"d03"}




"""**Save algorithm on the drive**"""

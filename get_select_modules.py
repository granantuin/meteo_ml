...
def label_y(df_all):
    ...
    print("Select independent variable number\n")
    for n,mod in zip(np.arange(0,len(y_variables)),y_variables):
        print(n,mod)
    y_number=int(input())    
    y_var=y_variables[y_number]
    #defining independent variable and label y
    #y_var = ["temp_o"] #@param ["[\"mod_o\"]", "[\"temp_o\"]", "[\"spd_o\"]", "[\"visibility_o\"]", "[\"dir_o\"]", "[\"dir_o\",\"spd_o\"]", "[\"wind_gust_o\"]", "[\"wxcodes_o\"]", "[\"skyc1_o\"]", "[\"skyl1_o\"]", "[\"temp_o\"]", "[\"tempd_o\"]", "[\"gust_spd_o\"]", "[\"prec_accumulated_1_hour_before_o\"]"] {type:"raw", allow-input: true}
    ...
    if y_var=="spd_o":
       interval,labels,Y = spd_beaufort (Y_raw,y_var) 
    ...
    #interval, labels, Y = select_var_f (y_var,Y_raw)
    #output Y labeled. Labels are category or string
    #Y = df_l[y_var[0]+"_l"]
    
    #show results
    df_all["Y_label"]=Y
    df_all[["Y_label",y_var]].sample(30)

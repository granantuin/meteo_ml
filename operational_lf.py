# -*- coding: utf-8 -*-
"""
Created on Mon Jan 31 12:41:59 2022
...
def select_algo():
    """
    select algorithm fileconver
    Returns
    -------
    algorithm object.
    """
    st.write("#### **Select algorithm**")  
    algorithms=[filename for filename in sorted(os.listdir("algo_La_Fanciulla/")) if filename.endswith('.al')]

        
    algorithm_file=st.selectbox("",(algorithms))
    algo=pickle.load(open("algo_La_Fanciulla/"+algorithm_file,"rb"))
...
def prob_result(algo): 
    """
    prob results
    """
       
    ml_prob= algo["model"].predict_proba(algo["pca"].transform(algo["scaler"].transform(df_x))) 
    ml_result=pd.DataFrame(ml_prob,columns=algo["model"].classes_).set_index(df_x.index)
...
def quality_report(algo):
    """
    Show quality reports in left side 
    Parameters
    ----------
    algo : algorith dictionary.
    Returns
    -------
    def_metmod_label variable .
    """
    ...
    return df_metmod_label

def deter_prob_forecast(algo,ml_result_c,df_metmod_label):
    """
    ...
    ml_result_c : Dataframe
        Probabilistic results from algorithm
    df_metmod_label : DataFrame
        Only when meteorological model gives direct output.
    Returns
    -------
...
def algo_features(algo):
    ...
    Returns
    -------
    Show algorithm features on screen.
    """
...
#select algorithm
algo,algorithm_file = select_algo()

#Show map
show_map(algo)

#get meteorological model
df_x=get_meteogalicia_model(algo) 

#select quality report
df_metmod_label=quality_report(algo)

#machine learning probabilistic result
ml_result_c = prob_result(algo)

#get raw metar
df_metar=get_metar(algorithm_file)

#Deterministic and probabilistic forecast
deter_prob_forecast(algo,ml_result_c,df_metmod_label)

#algorithm parameters and type
algo_features(algo)